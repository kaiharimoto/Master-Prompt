...
**A build that never publishes looks exactly like an updater that is broken.**
`publish-dev` fired only on the default branch, and every commit for a week
landed on a development branch: CI went green, the job skipped, and the release
page stayed on build 54. An installed copy checked at launch, correctly found
the same build it was already running, and said "you are on the newest build" —
which is the truth and completely useless. The condition now names the
development branch alongside the default one. If updates ever appear to stop,
**look at whether anything published before looking at the updater**: the app
reporting no update is the last link in the chain and the least likely to be
the broken one.
...
The icon is generated by `tool/make_icon.py` and committed alongside what it
produces, so it is reproducible rather than a binary nobody can regenerate.

**The letter is P, and Master Idea's is I.** The two are one family and look
it — same ink, same Inter, same hairline rule under the initial — so that
letter is the only thing telling them apart in a taskbar or a launcher. Both
drew M until this was said out loud, which made two identical icons for two
programs somebody is meant to run side by side.
